...
void Driver::moveToNextPos(Position targetPos){
	...
	while (abs(x - targetPos.first) > 10 * linearTolerance || abs(y - targetPos.second) > 10 * linearTolerance ){
		robot.read();
		x = robot.getXPos();
		y = robot.getYPos();
	}
	...
	while (abs(x - targetPos.first) > linearTolerance || abs(y - targetPos.second) > linearTolerance ){
		robot.read();
		x = robot.getXPos();
		y = robot.getYPos();
	}
